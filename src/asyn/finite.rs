...
/// let it = 0..2;
///
/// let (it2, a) = it.afmi_next().await.unwrap();
/// assert_eq!(a, 0);
///
/// let (it3, b) = it2.afmi_next().await.unwrap();
/// assert_eq!(b, 1);
///
/// let empty = it3.afmi_next().await;
...
/// {
///     while let Some((nextit, x)) = it.afmi_next().await {
///         it = nextit;
///         // Process `x`...
///     }
/// }
...
/// ...
/// 8  |     while let Some((nextit, x)) = it.afmi_next().await {
...
///    |
/// 78 |     async fn afmi_next(self) -> Option<(Self, Self::Item)>;
///    |                        ^^^^
/// ```
#[async_trait]
pub trait AsyncFiniteMoveIterator: Sized + Send {
    ...
    /// # trait T: Sized {
    /// # type Item;
    /// async fn afmi_next(self) -> Option<(Self, Self::Item)>;
    /// # }
    /// ```
    async fn afmi_next(self) -> Option<(Self, Self::Item)>;
}
    ...
    I: Iterator + Sized + Send,
{
    ...
    async fn afmi_next(mut self) -> Option<(Self, Self::Item)> {
