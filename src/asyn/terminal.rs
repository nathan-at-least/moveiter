...
/// let it = 0..2;
///
/// let (it2, a) = it.into_next().await.left().unwrap();
/// assert_eq!(a, 0);
///
/// let (it3, b) = it2.into_next().await.left().unwrap();
/// assert_eq!(b, 1);
///
/// let term = it3.into_next().await.right().unwrap();
/// assert_eq!((), term);
/// # });
/// ```
///
...
///     |
/// 142 |     async fn into_next(self) -> Either<(Self, Self::Item), Self::Terminal>;
///     |                        ^^^^
